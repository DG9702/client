...
const CourseContentEdit: FC<Props> = ({
  courseContentData,
  ...
  const handleAddTrack = (
    ...
    section.tracks.map((item: any, itemI: number) => {
      if (itemI === trackIndex) {
        positionUpdate = item?.position + 1;
      }
    });
    ...
    section.tracks.splice(trackIndex + 1, 0, newTrack);
  ...
  const handleDeleteTrack = (sectionIndex: number, trackIndex: number) => {
    ...
    const defaultValues =
      track.typeTrack === 'lesson'
        ? { title: '', description: '', videoUrl: '', duration: 0, links: [] }
        : {
            title: '',
            description: '',
            content: '',
    ...
    // Cập nhật các trường của track thành giá trị mặc định
    section.tracks[trackIndex] = {
      ...track,
      ...defaultValues
    };
                  ...
                  <>
                    {section.tracks.map((track: any, trackIndex: number) => (
                      <>
                        ...
                        <div>
                          <div className="w-[200px] relative flex justify-center border border-[#dee3e9] rounded items-center my-2">
                            <select
                              name=""
                              id="typeTrack"
                    ...
                    {/*add new content*/}
                    {index === sections.length - 1 && section.tracks.length === 0 && (
                      <div>
                        <div
                        ...
                        >
                          <select
                            ...
                            onChange={(e: any) => {
                              setTypeTrack(e.target.value);
                              handleAddTrack(index, 0, e.target.value);
                              setTypeTrack('');
                            }}
                          >
                            <option value="" className="dark:bg-black">
